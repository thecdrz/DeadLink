...
Client.exec = (command, options, callback) => {
  ...
  if(command === "gettime") {
    ...
    const jitter = () => {
      // small movement over time to simulate travel; bounded to ~\u00b12.0m
      const t = (Date.now() % 10000) / 10000;
      return (Math.sin(t * Math.PI * 2) * 2.0);
    };
    const P = (idx, name, x, y, z, health, deaths, zombies, level, ping) => {
      const jx = x + jitter();
      const jz = z + jitter() * 0.8;
      return `${idx}. id=${170+idx}, ${name}, pos=(${jx.toFixed(1)}, ${y.toFixed(1)}, ${jz.toFixed(1)}), rot=(0.0, 45.0, 0.0), remote=True, health=${health}, deaths=${deaths}, zombies=${zombies}, players=0, score=${Math.max(50, level*10 + zombies)}, level=${level}, steamid=7656119${100000000+idx}, ip=192.168.1.${100+idx}, ping=${ping}`;
    };
  ...
  // Emit a 'data' event so the main telnet handlers see activity (keeps watchdog quiet in dev-mode)
  try {
    Client.emit('data', Buffer.from(String(response || '') + "\n"));
  } catch(_) {}
