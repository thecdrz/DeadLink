...
function buildEmbed(kind, line) {
  const base = {
    footer: { text: `DeadLink • ${new Date().toLocaleString('en-US')}` }
  };
  if (kind === 'imminent') {
    return {
      color: 0xf39c12,
...
module.exports = function createBloodMoonMonitor(opts) {
  ...
  let timer = null;
  let lastState = 'idle';
  let lastCycleDay = null; // day number of last processed horde cycle
  let sentImminentForDay = false;
  let sentActiveForDay = false;
  let sentEndedForDay = false;
  ...
  const broadcastInGame = bmCfg.broadcastInGame !== false; // default on
    ...
    try {
      telnet.exec('gettime', { timeout: 5000 }, (err, response) => {
        ...
        // Determine current horde target day for this cycle
        if (time) {
          const hordeDay = (() => {
            const daysFromHorde = time.day % hordeFreq;
            if (daysFromHorde === 0) return time.day; // today is horde day
            return time.day + (hordeFreq - daysFromHorde); // next scheduled
          })();
          if (lastCycleDay === null || hordeDay !== lastCycleDay) {
            // New cycle; reset flags
            lastCycleDay = hordeDay;
            sentImminentForDay = false;
            sentActiveForDay = false;
            sentEndedForDay = false;
          }
        }

        if (state === 'imminent' && !sentImminentForDay) {
          const embed = buildEmbed('imminent', line);
          ch.send({ embeds: [embed] }).catch(() => {});
          if (broadcastInGame) {
            sayInGame('Blood Moon imminent! Horde begins in less than an hour.');
          }
          sentImminentForDay = true;
        }
        if (state === 'active' && !sentActiveForDay) {
          const embed = buildEmbed('active', line);
          ch.send({ embeds: [embed] }).catch(() => {});
          if (broadcastInGame) {
            sayInGame('The Blood Moon is active! Seek shelter immediately!');
          }
          sentActiveForDay = true;
        }
        if (state === 'idle' && lastState === 'active' && !sentEndedForDay) {
          const embed = buildEmbed('ended', line);
          ch.send({ embeds: [embed] }).catch(() => {});
          if (broadcastInGame) {
            sayInGame('The Blood Moon has ended. Regroup and rebuild.');
          }
          sentEndedForDay = true;
        }
      ...
      try { require('./lifecycle').registerTimeout(timer); } catch(_) {}
    }
  }
